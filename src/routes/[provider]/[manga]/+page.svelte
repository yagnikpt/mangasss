<script lang="ts">
  ...
  const { data } = $props()
  const providersIds = $derived(
    providers.map((p) => p.value).filter((p) => p !== data.provider),
  )
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-x-10 gap-y-4 mt-6">
      {#each data.chapters as chapter, index}
        <a
          ...
          class="block py-3 px-4 bg-neutral-800 rounded-md"
  ...
  {:else if data.status === 404}
    <div class="flex flex-col items-center gap-5">
      <p class="text-3xl font-medium">
        Manga not found for {data.provider.charAt(0).toUpperCase() +
