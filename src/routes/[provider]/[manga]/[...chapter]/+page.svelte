<script lang="ts">
  ...
  onMount(() => {
    ...
    mode =
      (localStorage.getItem('read_mode') as 'horizontal' | 'vertical') ??
      'horizontal'
      ...
      (entries) => {
        ...
        entries.forEach((entry) => {
          if (entry.intersectionRatio > 0 && entry.isIntersecting) {
            ...
            panels?.children[currentPage]?.children[1]?.removeAttribute(
              'loading',
            )
            panels?.children[currentPage + 1]?.children[1]?.removeAttribute(
              'loading',
            )
            syncProgress()
          }
  ...
  function handlePrevPage() {
    const pages = document.querySelectorAll('.page')
    if (currentPage !== 1)
      pages[currentPage - 2].scrollIntoView({ behavior: 'instant' })
  }
    ...
    {#if info.next}
      ...
      >
        ...
        <a
          ...
          class="block py-2 px-4 lg:text-lg bg-neutral-800 rounded-md"
          >Read next chapter</a
        >
      </div>
    {:else}
...
>
  ...
  <div class="flex w-full items-center justify-between">
    <img
      class="w-24"
      aria-hidden="true"
      src="/read_left_arrow.webp"
      alt="read left side image"
    />
  ...
  >
    ...
    <img
      ...
      src={`https://m3u8-proxy-cors-alpha-two.vercel.app/cors?url=${
        ...
        panel.headerForImage
          ? panel.headerForImage.Referer
          : refers[data.provider]
      }"}`}
      alt={panel.page}
      loading="lazy"
      onerror={(e) => {
